...
export const OrdersStats: FC = (): JSX.Element => {
	...
	//Готовые:
	const doneStatus = orders
		.filter((order) => order.status === "done")
		.filter((order, index) => index < 10);
	//В работе
	const inWorkStatus = orders
		.filter((order) => order.status !== "done")
		.filter((order, index) => index < 10);
	//Total && totalToday
		...
		<div className={styles.OrdersStats}>
			<ul className={`${styles.OrdersBoard}`}>
				<li className={`${styles.Order}`}>
					<h3
						className={`pb-6 text text_type_main-medium ${styles.OrderTitle}`}
					>
						Готовы:
					</h3>
					{doneStatus.length > 0 &&
						doneStatus.map((item, index) => (
							<p
								key={index}
								className={`pb-2 text text_type_digits-default ${styles.OrderDoneNumber}`}
							>
								{item.number}
							</p>
						))}
				...
				<li className={`${styles.Order}`}>
					...
					{inWorkStatus &&
						inWorkStatus.map((item, index) => (
							<p key={index} className={`pb-2 text text_type_digits-default `}>
								{item.number}
							</p>
						))}
				</li>
